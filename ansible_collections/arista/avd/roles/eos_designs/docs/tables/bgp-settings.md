...
=== "Table"
    ...
    | [<samp>&nbsp;&nbsp;mlag_ipv4_vrfs_peer</samp>](## "bgp_peer_groups.mlag_ipv4_vrfs_peer") | Dictionary |  |  |  | If the names of `mlag_ipv4_vrfs_peer.name` and `mlag_ipv4_underlay_peer.name` are same<br>then all the attributes of key `mlag_ipv4_vrfs_peer` will be ignored. |
...
=== "YAML"
    ...
    bgp_peer_groups:
      ...
      # then all the attributes of key `mlag_ipv4_vrfs_peer` will be ignored.
      mlag_ipv4_vrfs_peer:

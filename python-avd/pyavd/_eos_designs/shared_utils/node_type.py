...
class NodeTypeMixin(UtilsMixin):
    """
    ...
    def type(self) -> str:
        ...
        msg = f"'type' for host {self.shared_utils.hostname}"
        raise AristaAvdInvalidInputsError(msg)

    @cached_property
    def default_node_type(self) -> str | None:
        ...
        for default_node_type in self.inputs.default_node_types:
            for hostname_regex in default_node_type.match_hostnames:
                if search(f"^{hostname_regex}$", self.shared_utils.hostname):
    ...
    def connected_endpoints(self) -> bool:
        ...
        """
        return self.shared_utils.node_type_key_data.connected_endpoints
    ...
    def underlay_router(self) -> bool:
        ...
        """
        return self.shared_utils.node_type_key_data.underlay_router

    @cached_property
    def uplink_type(self) -> Literal["p2p", "port-channel", "p2p-vrfs", "lan"]:
        """
        Uplink type.

        uplink_type set based on <node_type_key>.nodes.[].uplink_type and node_type_keys.<node_type_key>.uplink_type.
        """
        return default(self.shared_utils.node_config.uplink_type, self.shared_utils.node_type_key_data.uplink_type)
    ...
    def network_services_l1(self) -> bool:
        ...
        """
        return self.shared_utils.node_type_key_data.network_services.l1
    ...
    def network_services_l2(self) -> bool:
        ...
        """
        return self.shared_utils.node_type_key_data.network_services.l2
    ...
    def network_services_l3(self) -> bool:
        ...
        if self.vtep and self.shared_utils.node_config.evpn_services_l2_only:
            return False
        return self.shared_utils.node_type_key_data.network_services.l3

    @cached_property
    def network_services_l2_as_subint(self) -> bool:
        """
    ...
    def mpls_lsr(self) -> bool:
        ...
        """
        return self.shared_utils.node_type_key_data.mpls_lsr
    ...
    def vtep(self) -> bool:
        ...
        """
        return default(self.shared_utils.node_config.vtep, self.shared_utils.node_type_key_data.vtep)

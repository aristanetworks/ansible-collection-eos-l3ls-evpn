...
def _create_avd_switch_facts_instances(all_inputs: dict[str, dict], pool_manager: PoolManager | None) -> dict:
    """
    ...
    Args:
        all_inputs: A dictionary where keys are hostnames and values are dictionaries of input variables per device.
            ...
            ```
        pool_manager: An instance of pyavd.PoolManager or subclass hereof implementing ".get_id(shared_utils: SharedUtils)".
            Used for dynamic ID allocations using the "pool_manager" feature.
    ...
    for hostname, hostvars in all_inputs.items():
        ...
        shared_utils = SharedUtils(hostvars=mapped_hostvars, templar=None, schema=EosDesignsAvdSchemaTools().avdschema, pool_manager=pool_manager)
